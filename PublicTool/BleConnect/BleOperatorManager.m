...
//  ModularizationPro
//
//  Created by DONGWANG on 2017/6/29.
...
/*!
 连接成功后确认服务的个数
 */
@property (nonatomic, assign) NSInteger serviceConfirmNum;
...
/*!
 *  discoveredTargetsArray容器中特定UUID对应的RSSI key:peripheral.identifier.UUIDString
 */
...
@property (nonatomic, strong) void(^sendDataWithRespond)(NSData *data);
@end
 ...
 */
-(void)sendDataToBand:(NSData *)data WithServiceUUID:(NSString *)serviceUUID WithCharacteristicUUID:(NSString *)characteristicUUID withWriteType:(CBCharacteristicWriteType)writeType  WithResult:(void(^)(NSData *data))resultBlock
{
    ...
    if ([[_peripheralServiceCharaDic allKeys] containsObject:serviceUUID])
    {
        ...
        if ([[charaDic allKeys] containsObject:characteristicUUID])
        {
            ...
            _sendDataWithRespond = resultBlock;
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error
{
    ...
    NSLog(@"didDiscoverServices Success");
...
//            NSLog(@"read characteristic.UUID~~~%@",characteristic.UUID);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self performSelectorOnMainThread:@selector(setStateInMainThreadOperate:) withObject:self.m_peripheral waitUntilDone:YES];
...
- (void) peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if(_sendDataWithRespond)
        _sendDataWithRespond(characteristic.value);
}
